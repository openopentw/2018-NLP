...
def sentence_to_embds (sentence, dic):
    """ Transform a sentence to embds.

    Args:
        sentence: A string that contain a sentence.
        dic: a dict file that maps a term to a vector.

    Returns:
        a (n * d) numpy-array,
        where n is the number of terms in sentence,
        and d is the dimension of the embedding from dic.
    """
    embds = []
...
if __name__ == '__main__':
    ...
    for tag_thres in [0, 1, 2, 3, 4]:
        ...
        np.savetxt('{}-{}.csv'.format(output_path, tag_thres), pred, delimiter=',')
