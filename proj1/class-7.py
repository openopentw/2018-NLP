...
def classify (y_data, thres=0.4):
    ...
    y_data_class[y_data > thres] = 2
    y_data_class[np.logical_and(y_data < thres, y_data > -thres)] = 1
    y_data_class[y_data < -thres] = 0
    return y_data_class
...
if __name__ == '__main__':
    ...
    y_train_class = classify(y_train[:,0])
    y_test_class = classify(y_test[:,0])

    print(X_train.shape)
    print(X_test.shape)

    dtrain = xgb.DMatrix(X_train, y_train_class)
    dtest = xgb.DMatrix(X_test)

    # print(X_train.shape)
    # print(x_test.shape)

    # Macro F1: 0.6424665164610913
    # Micro F1: 0.7318611987381702
    xgb_params = {
        'eta': 0.05,
        'max_depth': 6,
        'subsample': 0.6,
    ...
    pred_class = model.predict(dtest)

    # error = metrics.mean_squared_error(pred, y_test[:,0])
    # print('MSE: {}'.format(error))

    # pred_class = classify(pred)
    # TODO: change one-hot to -1 0 1

    np.savetxt(output_path, pred_class, delimiter=',')
